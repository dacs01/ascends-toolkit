    ...
    def post(self):
        ...
        if extn==".csv":
            ...
            try:
                ...
                response_to_send['path_to_data'] = file_path
...
class GetModelFileListHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        model_file_list = glob.glob("static/learned_models/*.pkl")
        response_to_send = {}
...
class GetPresetFileListHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        preset_file_list = glob.glob("static/config/*.*")
        response_to_send = {}
...
class ExecuteMLAnalysisHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if MAE!=-1:          
            asc.save_comparison_chart(predictions, actual_values, "static/output/ml/ml_result.png")
...
class SaveModelHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if model_abbr!='NET':
            ...
            asc.train_and_save(model, 'static/learned_models/'+tag+'.pkl', model_abbr
            ...
            optimizer = keras.optimizers.Adam(lr=lr)
            model = asc.net_define(params=net_structure, layer_n = layer, input_size = x_train.shape[1], dropout=dropout, l_2=l_2, optimizer=optimizer)
            asc.train_and_save_net(model, 'static/learned_models/'+tag+'.pkl', input_cols=header_x, target_col=header_y, x_train=x_train, y_train=y_train, scaler_option=scaler_option, MAE=MAE, R2=R2, path_to_save = '.', num_of_folds=5, epochs=epochs, batch_size=batch_size)

        model_files =  glob.glob("static/learned_models/*.pkl")
