...
def main(args):
    ...
    if auto_tune is True and model_type!='LRC':
    ...
    if test is True:
        ...
        try:
            ...
            if model_type!='NET':
                ...
                predictions, actual_values = asc.cross_val_predict_net_classifier(model, epochs=epochs, batch_size=batch_size, x_train = x_train, y_train = y_train, verbose = 0, scaler_option = scaler_option, num_of_folds = num_of_folds, num_of_class = num_of_class)
                accuracy = asc.evaluate_classifier(predictions, actual_values)
        ...
        print("* (%s)\t accuracy = %8.3f %d-fold cross validation "%(model_type, accuracy, num_of_folds))
    ...
    try:
        ...
        if model_type!='NET':
            model = asc.define_model_classifier(model_type, model_parameters, x_header_size = x_train.shape[1])
            asc.train_and_save_classifier(model, output_file+",Model="+model_type+",accuracy="+str(accuracy)+",Scaler="+scaler_option, model_type
...
if __name__=="__main__":
    ...
    print(" * Classifier ML model trainer \n")
    ...
    parser.add_argument( "--model_type", choices=['NET','RF','SVM','NN','RG','LRC'], default='RF', help="LRC (Logistic Regression), RF (Random Forest), SVM (Support Vector Machine), RG (Ridge), or NN (k-Nearest Neighbor), RF is selected by default")
